...
public class ModelPowerBlaze extends ModelBase
{
			...
			float par4, float par5, float par6, Entity par7Entity)
	{
		...
		for (i = 0; i < 4; ++i)
		{
			this.blazeSticks[i].rotationPointY = -2.0F
					+ MathHelper.cos((i * 2 + par3) * 0.25F);
		...
		for (i = 4; i < 8; ++i)
		{
			this.blazeSticks[i].rotationPointY = 2.0F + MathHelper
					.cos((i * 2 + par3) * 0.25F);
		...
		for (i = 8; i < 12; ++i)
		{
			this.blazeSticks[i].rotationPointY = 11.0F + MathHelper
					.cos((i * 1.5F + par3) * 0.5F);
