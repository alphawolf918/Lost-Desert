...
public class ModelLostDragon extends ModelBase
{
	...
	public ModelLostDragon(float par1)
	{
	...
	{
		GL11.glPushMatrix();
		EntityLostDragon entitydragon = (EntityLostDragon) par1Entity;
		float f6 = entitydragon.prevAnimTime
				+ (entitydragon.animTime - entitydragon.prevAnimTime)
				* this.partialTicks;
		...
		f7 = (f7 * f7 * 0.5F + f7 * 0.1F) * 0.05F;
		GL11.glTranslatef(0.0F, f7 - 2.0F, -3.0F);
		GL11.glRotatef(f7 * 2.0F, 1.0F, 0.0F, 0.0F);
		float f8 = -30.0F;
		float f9 = 0.0F;
		float f10 = 1.5F;
		double[] adouble = entitydragon
				.getMovementOffsets(6, this.partialTicks);
		float f11 = this.updateRotations(entitydragon.getMovementOffsets(5,
				this.partialTicks)[0]
				- entitydragon.getMovementOffsets(10, this.partialTicks)[0]);
		float f12 = this.updateRotations(entitydragon.getMovementOffsets(5,
				this.partialTicks)[0] + f11 / 2.0F);
		f8 += 2.0F;
		...
		for (int i = 0; i < 5; ++i)
		{
			double[] adouble1 = entitydragon.getMovementOffsets(5 - i,
					this.partialTicks);
			f15 = (float) Math.cos(i * 0.35F + f13) * 0.05F;
			this.spine.rotateAngleY = this.updateRotations(adouble1[0]
					- adouble[0])
					* (float) Math.PI / 180.0F * f10;
			this.spine.rotateAngleX = f15 + (float) (adouble1[1] - adouble[1])
					* (float) Math.PI / 180.0F * f10 * 5.0F;
			this.spine.rotateAngleZ = -this.updateRotations(adouble1[0] - f12)
					* (float) Math.PI / 180.0F * f10;
			...
			f14 = (float) (f14 - Math.cos(this.spine.rotateAngleY)
					* Math.cos(this.spine.rotateAngleX) * 10.0D);
			f9 = (float) (f9 - Math.sin(this.spine.rotateAngleY)
					* Math.cos(this.spine.rotateAngleX) * 10.0D);
		...
		double[] adouble2 = entitydragon.getMovementOffsets(0,
				this.partialTicks);
		this.head.rotateAngleY = this.updateRotations(adouble2[0] - adouble[0])
				* (float) Math.PI / 180.0F * 1.0F;
		this.head.rotateAngleZ = -this.updateRotations(adouble2[0] - f12)
				* (float) Math.PI / 180.0F * 1.0F;
		...
		for (int k = 0; k < 12; ++k)
		{
			adouble2 = entitydragon.getMovementOffsets(12 + k,
					this.partialTicks);
			f16 = (float) (f16 + Math.sin(k * 0.45F + f13) * 0.05000000074505806D);
			this.spine.rotateAngleY = (this.updateRotations(adouble2[0]
					- adouble[0])
					* f10 + 180.0F)
					* (float) Math.PI / 180.0F;
			this.spine.rotateAngleX = f16 + (float) (adouble2[1] - adouble[1])
					* (float) Math.PI / 180.0F * f10 * 5.0F;
			this.spine.rotateAngleZ = this.updateRotations(adouble2[0] - f12)
					* (float) Math.PI / 180.0F * f10;
			this.spine.rotationPointY = f8;
			this.spine.rotationPointZ = f14;
			this.spine.rotationPointX = f9;
