...
public class ChunkProviderTestDim implements IChunkProvider {
	...
	private MapGenStrongholdsLD strongholdGenerator = new MapGenStrongholdsLD();
	
	/** Holds Village Generator */
	private MapGenVillageLD villageGenerator = new MapGenVillageLD();
	...
	public ChunkProviderTestDim(World par1World, long par2, boolean par4) {
		caveGenerator = TerrainGen.getModdedMapGen(caveGenerator, CAVE);
		strongholdGenerator = (MapGenStrongholdsLD) TerrainGen.getModdedMapGen(
				strongholdGenerator, STRONGHOLD);
		villageGenerator = (MapGenVillageLD) TerrainGen.getModdedMapGen(
					...
					for (int l1 = 0; l1 < 8; ++l1) {
						...
						for (int i2 = 0; i2 < 4; ++i2) {
							...
							for (int k2 = 0; k2 < 4; ++k2) {
								if ((d16 += d15) > 0.0D) {
									par3ArrayOfByte[j2 += short1] = (byte) Blocks.purpleSand.blockID;
								} else if (k1 * 8 + l1 < b2) {
									par3ArrayOfByte[j2 += short1] = (byte) Blocks.creepStone.blockID;
								} else {
		...
		for (int k = 0; k < 16; ++k) {
			for (int l = 0; l < 16; ++l) {
				...
				int i1 = (int) (this.stoneNoise[k + l * 16] / 3.0D + 3.0D + this.rand
						.nextDouble() * 0.25D);
				int j1 = -1;
				byte b1 = (byte) Blocks.creepStone.blockID;
				byte b2 = (byte) Blocks.purpleSand.blockID;
					...
					if (k1 <= 0 + this.rand.nextInt(5)) {
						...
						if (b3 == 0) {
							...
							if (j1 == -1) {
								if (i1 <= 0) {
									b1 = 0;
									b2 = (byte) Blocks.purpleSand.blockID;
								} else if (k1 >= b0 - 4 && k1 <= b0 + 1) {
									b1 = (byte) Blocks.purpleSand.blockID;
									b2 = (byte) Blocks.creepStone.blockID;
								}
								...
								if (j1 == 0 && b2 == Block.sand.blockID) {
									...
									b2 = (byte) Blocks.purpleCaveStone.blockID;
								}
							}
						}
					}
	...
	public Chunk provideChunk(int par1, int par2) {
		this.rand.setSeed(par1 * 341873128712L + par2
				* 132897987541L);
		...
		for (int k2 = 0; k2 < par5; ++k2) {
			for (int l2 = 0; l2 < par7; ++l2) {
				...
				for (int k3 = 0; k3 < par6; ++k3) {
					...
					double d6 = 0.0D;
					double d7 = (k3 - d5) * 12.0D * 128.0D / 128.0D
							/ d4;
	...
	public void populate(IChunkProvider par1IChunkProvider, int par2, int par3) {
		...
		this.rand.setSeed(par2 * i1 + par3 * j1
						...
						par2, par3, flag, LAKE))
		{
			...
			(new WorldGenLakes(Blocks.purpleGlowstone.blockID)).generate(
					this.worldObj, this.rand, k1, l1, i2);
			
			k1 = k + this.rand.nextInt(16) + 10;
			l1 = this.rand.nextInt(128);
			i2 = l + this.rand.nextInt(16) + 10;
			...
			{
				k1 = k + this.rand.nextInt(16) + 10;
				l1 = this.rand.nextInt(this.rand.nextInt(120) + 10);
				i2 = l + this.rand.nextInt(16) + 10;
				
				if (l1 < 63 || this.rand.nextInt(10) == 0)
				{
					(new WorldGenLakes(Blocks.purpleCoalOre.blockID)).generate(
							this.worldObj, this.rand, k1, l1, i2);
				}
			}
		}
		...
		for (k1 = 0; doGen && k1 < 8; ++k1) {
			...
			(new WorldGenDungeonsLD()).generate(this.worldObj, this.rand, l1,
					i2, j2);
		}
	...
	public List getPossibleCreatures(EnumCreatureType par1EnumCreatureType,
		...
		return biomegenbase == null
				? null
				: (biomegenbase == Biomes.lostDesert
	...
	public ChunkPosition findClosestStructure(World par1World, String par2Str,
		...
		return "Stronghold".equals(par2Str) && this.strongholdGenerator != null
				? this.strongholdGenerator.getNearestInstance(par1World, par3,
						par4, par5) : null;
	}
