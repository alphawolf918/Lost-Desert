...
public class WorldGenDesertWellsLD extends WorldGenerator
{
	@Override
			...
			int par4, int par5)
	{
		...
		if (l != Blocks.purpleSand.blockID)
		{
			return false;
		}
		else
		{
				...
				for (j1 = -2; j1 <= 2; ++j1)
				{
					for (int k1 = -2; k1 <= 2; ++k1)
					{
						par1World.setBlock(par3 + j1, par4 + i1, par5 + k1,
								Blocks.purpleSandStone.blockID, 0, 2);
					}
				}
			}
			...
			par1World.setBlock(par3 + 2, par4 + 1, par5,
					Block.stoneSingleSlab.blockID, 1, 2);
			par1World.setBlock(par3 - 2, par4 + 1, par5,
					Block.stoneSingleSlab.blockID, 1, 2);
