...
public class MapGenStrongholdsLD extends MapGenStructure
{
	public static ArrayList<BiomeGenBase> allowedBiomes = new ArrayList<BiomeGenBase>(
			Arrays.asList(Biomes.lostDesert));
	...
	public MapGenStrongholdsLD(Map par1Map)
	{
		this.allowedBiomeGenBases = allowedBiomes.toArray(new BiomeGenBase[0]);
		this.structureCoords = new ChunkCoordIntPair[3];
		this.field_82671_h = 32.0D;
	...
	protected boolean canSpawnStructureAtCoords(int par1, int par2)
	{
		if (!this.ranBiomeCheck)
		{
			...
			{
				double d1 = (1.25D * k + random.nextDouble())
						* this.field_82671_h * k;
				...
				ChunkPosition chunkposition = this.worldObj
						.getWorldChunkManager().findBiomePosition(
								(i1 << 4) + 8, (j1 << 4) + 8, 112, arraylist,
								random);
				...
				d0 += (Math.PI * 2D) * k / this.field_82672_i;
	...
	protected StructureStart getStructureStart(int par1, int par2)
	{
		...
		for (structurestrongholdstart = new StructureStrongholdStart(
				this.worldObj, this.rand, par1, par2); structurestrongholdstart
				.getComponents().isEmpty()
				|| ((ComponentStrongholdStairs2) structurestrongholdstart
						.getComponents().get(0)).strongholdPortalRoom == null; structurestrongholdstart = new StructureStrongholdStart(
